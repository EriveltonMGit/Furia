...
import { runTransaction, doc, updateDoc, Transaction } from "firebase/firestore";
...
export class FaceVerificationService {
    ...
    static async verifyFaceMatch(idDocument: File, selfie: File, userId?: string): Promise<VerificationResponse> {
        try {
            ...
            if (userId && result.success && result.faceVerified) {
...
export const verifyIdentity = async (
    ...
    try {
        ...
        // Se tiver userId e a verificação for bem-sucedida, salva o resultado no banco
        if (userId && result.success && result.faceVerified) {
            await FaceVerificationService.saveVerificationResult(userId, result);
        }
